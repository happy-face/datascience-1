...
def parse_args():
    ...
    parser.add_argument("-i", "--input", required=True, nargs = 2, help="Input CSV training and test dataset files")
...
if __name__ == "__main__":
    ...
    df_training = pd.read_csv(args.input[0], nrows=args.max_samples)
    df_training.head()

    df_test = pd.read_csv(args.input[1], nrows=args.max_samples)
    df_test.head()
    ...
    for n in df_training.main_categories:
        unique_categories.update(n)
    category_to_id = dict([(j,i) for i, j in enumerate(sorted(unique_categories))])


    y_df_training = df_training['main_categories'].apply(one_hot_encoder)
    y_df_training = pd.DataFrame(y_df_training.values.tolist(), columns=range(0, len(category_to_id)))
    ...
    x1_train = df_training['title'].values
    x2_train = df_training['abstract'].values
    y_train = y_df_training.values
